...
Functional V1 emits its own fixed Tire `Advisory.Readiness` actuator from actual telemetry readiness: on a boolean transition at the next worker iteration, plus a five-second heartbeat. Successful writes are at least 100 ms apart; unaccepted/uncertain writes retry after one second with the current value. Only a successful RPC without KUKSA top-level or entry errors acknowledges publication. A renewed authenticated session starts independently; it never inherits ready status to hide absent data. Freshness, model thresholds and credentials are unchanged. It polls only `/api/v1/tire/demo-control/poll` on its existing backend. No inbound control listener is added.
